...
assign pos_data = multiplicand;

assign neg_data = ~multiplicand;
...
assign correction = sub;

wire [31:0] shifted_data;

wire [31:0] shifted_corr;

assign shifted_data = {{16{selected_data[15]}},selected_data} << shift;

assign shifted_corr = {{31{1'b0}},correction} << shift;

ripple_adder_32 corr_add(
...
booth_encoder be0 (.y_i(multiplier[0]), .y_im1(y_m1), .add(add0), .sub(sub0));
...
booth_partial_product ppg12 (.multiplicand(multipand), .add(add12), .sub(sub12), .shift(5'd12), .pp(pp12));
...
ripple_adder_32 a0_3 (.a(s01), .b(s23), .sum(s0_3));

ripple_adder_32 a4_7 (.a(s45), .b(s67), .sum(s4_7));

ripple_adder_32 a8_11 (.a(s89), .b(s1011), .sum(s8_11));

ripple_adder_32 a12_15 (.a(s1213), .b(s1415), .sum(s12_15));

wire [31:0] s0_7;

wire [31:0] s8_15;

ripple_adder_32 a0_7 (.a(s0_3), .b(s4_7), .sum(s0_7));

ripple_adder_32 a8_15 (.a(s8_11), .b(s12_15), .sum(s8_15));

ripple_adder_32 final_add(
